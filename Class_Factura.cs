...
namespace ProyectoBarRestaurant
{
    internal class Class_Factura
    {
       ...
       public Class_Factura()
        ...
        //public void ImprimirFactura(DataGridView dataGridView, string numero_Factura, string numero_Mesa,
        //                       string nombre, string cedula, string direccion,
        //                       string celular, string correo_electronico, string total_final)
        //{
        //    conexion.OpenConnection();
        ...
        //        string rutaArchivo = @"C:\Users\Leandro\Desktop\Borrador\Factura" + numero_Factura + ".txt";

        //        // Crear o sobrescribir el archivo de texto
        //        using (StreamWriter escritor = new StreamWriter(rutaArchivo))
        //        {
        //            escritor.WriteLine("********************************************************************************************************");
        //            escritor.WriteLine("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t FACTURA");
        //            escritor.WriteLine("********************************************************************************************************");
        //            escritor.WriteLine("Número de Factura: " + numero_Factura);
        //            escritor.WriteLine("Número de Mesa: " + numero_Mesa);
        //            escritor.WriteLine("Nombre: " + nombre);
        //            escritor.WriteLine("Cédula: " + cedula);
        //            escritor.WriteLine("Dirección: " + direccion);
        //            escritor.WriteLine("Celular: " + celular);
        //            escritor.WriteLine("Correo Electrónico: " + correo_electronico);
        //            escritor.WriteLine("********************************************************************************************************");
        //            escritor.WriteLine("Descripción\t\tCantidad\tPrecio Unitario\tTotal");
        //            escritor.WriteLine("********************************************************************************************************");

        //            string consultaSql = @"SELECT Menu.Descripcion AS NombreMenu
        //                       FROM MESA
        //                       INNER JOIN Menu ON MESA.Codigo_Menu = Menu.Codigo
        //                       WHERE NumeroMesa = @NumeroMesa";

        //            SqlCommand cmdConsulta = new SqlCommand(consultaSql, conexion.Connection);
        //            cmdConsulta.Parameters.AddWithValue("@NumeroMesa", numero_Mesa);

        //            SqlDataReader reader = cmdConsulta.ExecuteReader();

        //            string descripcionMenu = string.Empty;

        //            if (reader.Read())
        //            {
        //                descripcionMenu = reader["NombreMenu"].ToString();
        //            }
        ...
        //            foreach (DataGridViewRow row in dataGridView.Rows)
        //            {
        //                descripcionMenu = row.Cells["NombreMenu"].Value.ToString();
        //                int cantidadMenu = Convert.ToInt32(row.Cells["Cantidad_Menu"].Value);
        //                decimal precioUnitario = Convert.ToDecimal(row.Cells["Precio_Unitario"].Value);
        //                decimal total = Convert.ToDecimal(row.Cells["Total"].Value);
        ...
        //                // Escribir detalles en el archivo de texto con formato cuadriculado
        //                escritor.WriteLine($"| {descripcionMenu,-20} | {cantidadMenu,-10} | {precioUnitario.ToString("C2"),-15} | {total.ToString("C2"),-15}");


        //            }
        //            // Línea de separación antes del Total Final
        //            escritor.WriteLine("+----------------------------------------+------------+---------------+---------------+");

        //            // Escribir línea del Total Final
        //            escritor.WriteLine($"| {"Total Final:",81} | {total_final,-15} |");
        //            escritor.WriteLine("+----------------------------------------+------------+---------------+---------------+");
        ...
        //    catch (Exception ex)
        //    {
        //        MessageBox.Show("Ocurrió un error durante la impresión: " + ex.Message);
        //    }
        //    finally
        //    {
        //        conexion.CloseConnection();
